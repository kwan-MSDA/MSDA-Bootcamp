...
+ An object is a data structure that contains data and metadata about the data
...
+ R has several fundamental object classes that determine how data is organized and used
  + vector
    + the most basic object class in R
    + a one-dimensional array that can contain elements of the same type
    + can be created using the `c()` function
    + There are four basic types of vectors in R
      + characters, numerics, logicals, and factors

::: notes

+ characters (i.e. strings)
+ numerics (i.e. numbers)
+ logicals (i.e. TRUE/FALSE)
+ factors (i.e. categorical variables)

integer (e.g., c(1L, 2L))
complex (e.g., c(1+2i, 3+4i))
raw (for bytes, less common)

+ An object is a data structure that contains data and metadata about the data
  + Metadata is information about the data, such as its type, length, and attributes
  + Objects can be created using functions such as `c()`, `data.frame()`, and `matrix()`

+ Factor: Used for categorical data, storing levels as integers with labels. Example: factor(c("low", "high", "low")).

+ Tibble: A modern version of a data frame (from the tibble package), with enhanced printing and stricter behavior
...
+ R has several fundamental object classes that determine how data is organized and used
  + matrix
    + a two-dimensional array that can contain elements of the same type
    + can be created using the `matrix()` function
  + data frame
    + a two-dimensional array that can contain elements of different types
    + can be created using the `data.frame()` function
  + list
    + a collection of objects that can contain elements of different types
    + can be created using the `list()` function
    + lists can also contain other lists

+ R has other object classes as well - e.g. arrays, functions, and environments

::: notes

+ Function: Objects that represent executable code (e.g., function(x) x + 1).

+ S3 and S4 Objects: These are used in R’s object-oriented systems:
  + S3: Informal, attaches a class attribute to an object (e.g., lm() output has class "lm").
  + S4: More formal, with defined slots and stricter rules (e.g., objects from packages like Bioconductor).

+ Environment: Objects that store variable bindings (e.g., .GlobalEnv for the global workspace).

+ Special Classes: Less common classes like ts (time series), formula (e.g., y ~ x), or package-specific classes (e.g., ggplot objects)

+ Functions are also objects in R

+ factors are stored as integers under the hood, but have a "label" attribute that maps the integers to the corresponding category names
    + factors are useful for statistical modeling and data visualization
    + they allow R to treat categorical variables appropriately
...
```{r}
#| echo: true
#| code-summary: alternative multiplication operator
...
#| echo: true
#| eval: false
#| code-summary: transpose of a matrix
...
#| class: fragment
#| eval: false
#| code-summary: convert to character

agevec<- as.character(agevec)
...
#| class: fragment
#| eval: false
#| code-summary: another example of changing data type
...
#| class: fragment
#| eval: false
#| code-summary: convert to factor
...
genderfactorvec<- as.factor(gendervec)

class(genderfactorvec)

typeof(genderfactorvec)
...
#| echo: true
#| eval: false
#| code-summary: create two data frames
#| class: fragment
...
starwars[, 1] ## Get the 1st column of the starwar s data frame
...
+ go to <a href="https://raw.githubusercontent.com/kwan-MSDA/MSDA-Bootcamp/main/dataset/UNpop.csv" download="UNpop.csv">here</a> to download a sample dataset
